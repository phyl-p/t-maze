...
parameters = struct(...
    ...
    'toggle_testing' ,              true,...
    'number_of_trials',             50,... 
    'consecutive_successes_to_halt',0,... % after this many successes-in-a-row, halt the program (this saves time); will only run if the previous toggle is on.
    ...
    ... % boxcar settings
    ...
    'boxcar_scales',                [0.5 0.5],... % should sum to 1
    'boxcar_window_inh',            2,...
    'boxcar_scales_inh',            [0.5 0.5],...
    ...
    ... % random number seeds
    'network_construction_seed',    randi(100,1),...26,...58,...%randi(100,1),... 
    'z_0_seed',                     randi(100,1),...95,...%randi(100,1),... 
    ...
    ... % network topology settings
    'number_of_neurons',            uint16(2500),...
    'connectivity',                 0.1,... %fan in is exact i.e. each postsyn. nrn is enervated by same number of nrns
    'desired_mean_z',               0.085,...
    'gradient',                     -0.25,... % currently disabled
    ...
    ...
    'off_noise',                    0.25,... %probability between 0-1
    'test_off_noise',               0.25,...
    'test_on_noise',                0.0,...
    ...
    ... % testing parameters
    ...
    'ext_activation',               uint16(32),... %default 30
    'stutter',                      uint16(5),...x
    'shift',                        uint16(16),... %default 15, must be less than 30 (?)
    'paradigm_left',                [5 4 3 2 2 2 2 1 1 1 1 1 1 1 1 ],...%[15 8 6 6 4 4 2 2 2 1 1 1 1],... %[16 12 5 3 3 2 2 1 1 1 1 1],...[13,3,2], ...[20 8 3 2 1 1 1],...seed 06[20 8 3 2 1 1 1], ...[13,3,2],...[20 12 6 4 3 2 2 2 1 1 1 1 1],...[15,5,4,2,1],...[20 12 6 4 3 2 1 1 1],...%seed02 dual[13,3,1],...%seed02 single[20 12 6 3], ...[13,3,2,1],...
    'paradigm_right',               [5 4 3 2 2 2 2 1 1 1 1 1 1 1 1 ],...
    'num_start_block',              uint16(0),...
    'num_shared_block',             uint16(12),...
    'num_end_block',                uint16(20),...
    'attr_percent_ext_nrns',        0.1,... % specify the percentage of externals block that are attractors
    'toggle_training_attractor',    false,...
    'extra_train_second_pattern_trials' ,   0,...
    'num_firing_before_choice_point',      5,... used in determinechoicepointcounts
    'determine_choice_point_duration', 20,... used in DetermineTMazeSuccess, set
    'choice_point_blocks',          3,...used in DetermineTMazeSuccess, sets how many blocks count as cp blocks
    ... % test I-Maze params
    'test_num_stimuli',              uint16(12),... $ equivalent to first_stimulus_length for straight sequence
    'num_test_pattern',              uint16(6),... % default to 4 as there are four patterns in total          
    'test_reps_for_each_pattern',    uint16(10),...
    ...
    ... % modification rates
    'toggle_activity_coerce',       true,... %decides whether to forcefully increase/reduce activity
    'bound_percent',                0.1,...
    ...
    'epsilon_pre_then_post',        0.003, ...%0.005,...
    'epsilon_post_then_pre',        0.003,...
    'epsilon_feedback',             0.3,...
    ...
    'epsilon_k_ff',                 0.0,...%kff will not be updated unless pre then post is off; kff is set to 1/3 of k0 for each trail, set in run model
    ... 
    ...
    'fractional_mem_pre',           0.72,... %nmda; used with pre_then_post, controls Zbar pre decay, -1/log(fractional) = exponential time constant
    'fractional_mem_post',          0.72,... %spiketiming; used with post_then_pre, controls Zbar post decay
    'offset_pre_then_post',         2,...
    'offset_post_then_pre',         0,...
    ...
    ... % starting values for some variables
    'k_0_start',                    0.05,...%0.3,...  %old 0.4
    'k_fb_start',                   0.025,...0.4,...0.0,...% old 0.04
    'k_ff_start',                   0.005*0.67*0.5,...0.005,...0.0146,... 0.0066 ...0.008
    ...
    ... % weight settings
    'toggle_rand_weights',          false,... %toggle between random and uniform excitatory weights
    'weight_start',                 0.3,... %starting value if using uniform weights
    'weight_high',                  0.4,... %upper limit for weight values if using random distribution
    'weight_low',                   0.2,... %lower limit for weight values if using random distribution
...
function parameters = AdjustParameters(parameters)
    ...
    externals = parameters.ext_activation;
    shift = parameters.shift;
    num_start_blocks = parameters.num_start_block;
    num_shared_blocks = parameters.num_shared_block;
    num_end_blocks = parameters.num_end_block;
    ...
    parameters.nrn_viewing_range = [1,total_external_nrns + 100];
    %end
end
